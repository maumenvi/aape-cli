...
The `trusted` source flag is an explicit provenance decision recorded in `.maia/maia.lock.json`. It is not a sandbox, signature, malware scan, or guarantee that a package is safe. Remote sources default to `trusted: false`; Maia's bundled local registry is trusted because it is shipped with the installed CLI.
